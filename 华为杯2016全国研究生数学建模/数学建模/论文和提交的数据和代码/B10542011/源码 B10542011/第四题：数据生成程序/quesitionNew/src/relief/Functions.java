...
public class Functions
{
	...
	ArrayList<Integer> biaoshi=new ArrayList<Integer>();
	int N=9445;//样本的特点
	int T=1000;//样本的个数/迭代次数 （一般和训练集样本个数相同）
	//降维 
	public  void CalMAF(float yuzhi,String path,String pathid,String pathXZtype)
	{
		init(path,pathXZtype);
		Sample head=al.get(0);
		String [] headName=head.getGenetype().split("\\s++");
		Sample tail=al.get(1);
		...
		for(int i=0;i<N;i++)//位点
		{
			...
			char tag1=gene.charAt(0);
			 rate1++;
			 int bs1=biaoshi.get(0);
			 int bs2=biaoshi.get(1);
			...
			for(int j=2;j<1000;j++)
			{
				...
				Sample sample=al.get(j);
				 ...
				 if(gene.charAt(0)==tag1&&biaoshi.get(j)==bs1)
					 rate1++;
				 if(gene.charAt(1)==tag1&&biaoshi.get(j)==bs1)
					 rate1++;
				 
				 if(gene.charAt(0)==tag2&&biaoshi.get(j)==bs2)
					 rate2++;
				 if(gene.charAt(1)==tag2&&biaoshi.get(j)==bs2)
					 rate2++;
	...
	public ArrayList<Neighbor> getSameNearstId(Sample training)
	{
		...
		/* if(training.isTag()==1)//表示训练集是有病的状态
		 {
		 ...
		 Neighbors = SameCase(training, Neighbors, id);
		//排序按照 weight相关性大小排序
		 Collections.sort(Neighbors,comparator);
	...
	//在相同类型的样本
		private ArrayList<Neighbor> SameCase(Sample training, ArrayList<Neighbor> Neighbors,
				int id) 
	      {
		...
		//在不同类型的样本
				private ArrayList<Neighbor> DifferentCase(Sample training, ArrayList<Neighbor> Neighbors,
						int id) 
			      {
					int tag=training.isTag();
					int sameGene=0;//相同的基因组
	...
	public ArrayList<Neighbor> getDistinctNearstId(Sample training)
	{
		...
		ArrayList<Neighbor> Neighbors=new ArrayList<Neighbor>();
		 int id=training.getId()-1;
		// int sameGene=0;//相同的基因组
		/* if(training.isTag()==0)//表示训练集是无病的状态
		 {
			 Neighbors = lastCase(training, Neighbors, id);
			 //排序按照 weight相关性大小排序
			 Collections.sort(Neighbors,comparator);
		 ...
		 Neighbors =DifferentCase(training, Neighbors, id);
		//排序按照 weight相关性大小排序
		 Collections.sort(Neighbors,comparator);
	...
	//算法主程序,path是路径，k是控制测试的近邻个数
	public void mainProcedure(String path,String pathXZtype,int k,String outpath,String pathimportid)
	{
		init(path,pathXZtype);
	...
	/*	for(int i=0;i<Neighbors.size();i++)
		{
			System.out.println(Neighbors.get(i).getId()+":"+Neighbors.get(i).getWeight());
		}*/
  ...
  //产生性状的标识  0:表示没  1：表示有
     public void producetag(String pathXZtype)
     {
    	 FileInputStream fis=null;
		 InputStreamReader isr=null;
		 BufferedReader br=null;
		...
		try
		{
			 ...
			 fis=new FileInputStream(pathXZtype);
			 isr=new InputStreamReader(fis);
			 br=new BufferedReader(isr);
			 String str="";
			 while((str=br.readLine())!=null)
			 {
				 ...
				 biaoshi.add(Integer.parseInt(str));
			 }
	...
	public void init(String path,String pathXZtype)
	{
	
		 producetag(pathXZtype);
		 FileInputStream fis=null;
		 InputStreamReader isr=null;
		 BufferedReader br=null;
		try
...
//获取在不同类别中的近邻id
/*	public ArrayList<Neighbor> getDistinctNearstIdNew(Sample training)
	{
